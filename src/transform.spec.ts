...
describe('transform', () => {
    ...
    it('should run the transform pipeline', async () => {
        const transformResult: GeoLoc[] = [];
        await pipeline(
        ...
        expect(transformResult).toEqual([
            { city: 'Youngstown', state: 'OH', geometry: <GeoLoc>{ type: 'Point', coordinates: [-80.65, 41.0997] } },
            { city: 'Yankton', state: 'SD', geometry:  <GeoLoc> { type: 'Point', coordinates: [-97.3897, 42.88] } },
            { city: 'Yakima', state: 'WA', geometry: <GeoLoc>{ type: 'Point', coordinates: [-120.51, 46.5997] } },
            { city: 'Worcester', state: 'MA', geometry: <GeoLoc>{ type: 'Point', coordinates: [-71.8, 42.27] } },
            { city: 'Wisconsin Dells', state: 'WI', geometry: <GeoLoc>{ type: 'Point', coordinates: [-89.7697, 43.63] } },
            { city: 'Winston-Salem', state: 'NC', geometry: <GeoLoc>{ type: 'Point', coordinates: [-80.25, 36.0997] } },
            { city: 'Winnipeg', state: 'MB', geometry: <GeoLoc>{ type: 'Point', coordinates: [-97.15, 49.88] } },
        ]);
    });
});
